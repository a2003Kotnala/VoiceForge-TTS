...
exports.suggestEmotion = suggestEmotion;
...
function resolveSupportedLanguage(targetCode, supportedLanguages) {
    ...
    return (supportedLanguages.find((languageCode) => languageCode.startsWith(`${baseLanguage}-`)) ?? supportedLanguages[0]);
}
function scoreKeywordLanguages(tokens) {
    return LANGUAGE_PROFILES.filter((profile) => profile.keywords?.length).map((profile) => ({
        profile,
        score: profile.keywords?.reduce((count, keyword) => count + Number(tokens.includes(keyword)), 0) ?? 0
    }));
}
function detectLanguage(inputText, supportedLanguages) {
    ...
    if (winner && winner.score > 0) {
        const confidence = winner.score >= 4 || text.length > 120
            ? "high"
            : winner.score >= 2 || text.length > 40
                ? "medium"
                : "low";
        return {
            code: resolveSupportedLanguage(winner.profile.defaultCode, supportedLanguages),
